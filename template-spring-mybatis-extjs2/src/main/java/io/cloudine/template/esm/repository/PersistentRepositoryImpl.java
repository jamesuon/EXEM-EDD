 ...
 */
package io.cloudine.template.esm.repository;
...
public abstract class PersistentRepositoryImpl<D, P> extends DefaultSqlSessionDaoSupport implements PersistentRepository<D, P> {
    ...
    @Override
    public D selectById(P identifier) {
        return this.getSqlSessionTemplate().selectOne(this.getNamespace() + ".selectById", identifier);
    }
    ...
    @Override
    public ArrayList<D> selectList() {
        return (ArrayList<D>)this.getSqlSessionTemplate().selectList(this.getNamespace() + ".selectList");
    }
}
